...
export const servicesRouter = createTRPCRouter({
    ...
    byCategory: publicProcedure
        ...
        .query(async ({ ctx, input }) => {
            const { slug, page, limit, minPrice, maxPrice, minRating, search } = input;
            const offset = (page - 1) * limit;
            ...
            const conditions = [eq(services.categoryId, category.id)];
            ...
            // 4️⃣ Paginated items with filters
            const items = await ctx.db
                .select({
                    id: services.id,
                    title: services.title,
